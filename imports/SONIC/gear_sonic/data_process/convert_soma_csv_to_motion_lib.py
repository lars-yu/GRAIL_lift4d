...
def load_bones_csv(csv_path: str) -> dict:
    ...
    # Read by explicit joint name instead of relying on dataframe column order.
    # Some exports use the raw USD/articulation joint name without the "_dof"
    # suffix, so accept both spellings.
    joint_cols = []
    missing_joint_cols = []
    for joint_name in BONES_CSV_JOINT_NAMES:
        raw_joint_name = joint_name.removesuffix("_dof")
        candidates = [joint_name, raw_joint_name]
        for candidate in candidates:
            if candidate in data.columns:
                joint_cols.append(candidate)
                break
        else:
            missing_joint_cols.append(joint_name)

    if missing_joint_cols:
        available_dof_cols = [c for c in data.columns if c.endswith("_dof") or c.endswith("_joint")]
        raise KeyError(
            f"{csv_path} is missing {len(missing_joint_cols)} required joint columns. "
            f"First missing: {missing_joint_cols[:5]}. "
            f"Available joint-like columns: {available_dof_cols[:10]}"
        )
