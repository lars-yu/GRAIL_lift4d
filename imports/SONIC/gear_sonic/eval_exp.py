    ...
    def evaluate_checkpoint(
        ...
        try:
            ...
            if mode == "metrics":
                ...
                # The saved training config.yaml resolves `manager_env.recorders` to a
                # bare `RecordersCfg` (no terms). The defaults-list dance
                # `~manager_env/recorders +manager_env/recorders=render` only works
                # when Hydra is composing from a defaults list; on a resolved config
                # snapshot it no-ops, leaving the recorder manager empty and producing
                # zero videos. Inject the render_envs term directly via dot overrides.
                cmd += (
                    " ++manager_env.recorders.render_envs._target_="
                    "gear_sonic.envs.manager_env.mdp.recorders.RenderEnvsRecorderCfg"
                    f" ++manager_env.recorders.render_envs.video_save_path={work_dir}/render_results"
