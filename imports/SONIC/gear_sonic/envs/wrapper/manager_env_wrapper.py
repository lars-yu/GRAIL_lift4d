...
class ManagerEnvWrapper:
    ...
    def _setup_replay_joint_indices(self):
        """Set replay indices from the command mapping, with legacy G1 fallback."""
        if self.motion_command is not None and getattr(
            self.motion_command, "has_dof_mismatch", False
        ):
            self._body_joint_indices = self.motion_command.body_joint_indices
            self._hand_joint_indices = self.motion_command.extra_joint_indices
            return
    ...
    def _setup_policy_atm(self, env_config, algo_config):  # noqa: ARG002
        ...
        # Policies produce action-term DOFs, which may be fewer than articulation
        # joints (X2 exposes passive finger joints in the USD).
        self._current_num_joints = self.env.action_manager.total_action_dim
    ...
    def _update_replay_frame(self):
        ...
        controlled_joint_indices = getattr(
            self.motion_command, "body_joint_indices", self._body_joint_indices
        )
        if robot_num_joints > motion_lib_num_dof and controlled_joint_indices is not None:
            ...
            # Start from articulation defaults so passive joints retain the USD pose.
            ...
            # Map reference DOFs through the command's name-resolved indices.
            joint_pos[:, controlled_joint_indices] = motion_lib_joint_pos
            joint_vel[:, controlled_joint_indices] = motion_lib_joint_vel

            # Use hand DOFs from motion lib if available, otherwise default to zero.
            # hand_dof_pos is stored in G1_HAND_JOINTS order; write it back through
            # the name-based hand indices instead of assuming hands are the final
            # robot joints in articulation order.
            ...
            if hand_dof_pos is not None:
                ...
                if (
                    self._hand_joint_indices is None
                    or len(self._hand_joint_indices) < num_hand_dof
                ):
                    num_hand_indices = (
                        0
                        if self._hand_joint_indices is None
                        else len(self._hand_joint_indices)
                    )
                    raise RuntimeError(
                        "Replay motion contains hand DOFs but robot hand joint indices "
                        f"are unavailable or incomplete: got "
                        f"{num_hand_indices} indices for {num_hand_dof} hand DOFs."
                    )
                joint_pos[:, self._hand_joint_indices[:num_hand_dof]] = hand_dof_pos
        else:
