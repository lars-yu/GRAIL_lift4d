...
def get_active_object(env):
    """Return the active object asset from the scene.

    In multi-object-per-env mode (num_obj_per_env > 1), the active object is
    ``object_0``. In the legacy single-object mode, it is ``object``.
    """
    if "object_0" in env.scene.rigid_objects:
        return env.scene["object_0"]
    return env.scene["object"]


def get_active_object_name(env) -> str:
    """Return the scene key of the active object."""
    if "object_0" in env.scene.rigid_objects:
        return "object_0"
    return "object"
