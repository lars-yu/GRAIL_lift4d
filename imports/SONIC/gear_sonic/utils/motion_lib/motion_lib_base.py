...
class MotionLibBase:
    ...
    def load_motion_with_skeleton(
        ...
        for f in pbar:
            ...
            if curr_file.get("robot") == "agibot_x2_omnihand":
                required_metadata = {"dof_names", "motion_body_names"}
                missing_metadata = sorted(required_metadata - set(curr_file))
                if missing_metadata:
                    raise ValueError(
                        f"X2 motion index {curr_id} is missing metadata "
                        f"{missing_metadata}; regenerate it with the X2 retargeter"
                    )
                if list(curr_file["dof_names"]) != self.mesh_parsers.actuated_joint_names:
                    raise ValueError(
                        "X2 motion DOF names do not match the configured MJCF; "
                        "regenerate the motion or the IK asset"
                    )
                if list(curr_file["motion_body_names"]) != self.mesh_parsers.body_names:
                    raise ValueError(
                        "X2 motion body names do not match the configured MJCF; "
                        "fixed/passive finger bodies would be misaligned"
                    )
                if pose_aa.shape[1] != len(self.mesh_parsers.body_names):
                    raise ValueError(
                        f"X2 pose_aa has {pose_aa.shape[1]} bodies, expected "
                        f"{len(self.mesh_parsers.body_names)}"
                    )
            ...
            if self.mesh_parsers is not None:
                ...
                if hand_dof_count > 0 and "dof" in curr_file:
                    ...
                    if raw_dof.shape[-1] > 29:
                        hand_dof_input_order = self.m_cfg.get(
                            "hand_dof_input_order", "g1_hand"
                        )
                        valid_hand_orders = {"g1_hand", "grouped", "articulation", "isaaclab"}
                        if hand_dof_input_order not in valid_hand_orders:
                            raise ValueError(
                                "Unsupported hand_dof_input_order="
                                f"{hand_dof_input_order!r}. Expected one of "
                                f"{sorted(valid_hand_orders)}."
                            )
