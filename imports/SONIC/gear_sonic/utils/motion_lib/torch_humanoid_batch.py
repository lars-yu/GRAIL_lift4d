...
class Humanoid_Batch:
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        joint_elements = tree.getroot().find("worldbody").findall(".//joint")
        joints = sorted([joint.attrib["name"] for joint in joint_elements])
        motor_elements = list(tree.getroot().find("actuator").getchildren())
        motors = sorted([motor.attrib["name"] for motor in motor_elements])
        self._actuated_joint_names = {
            motor.attrib.get("joint", motor.attrib["name"]) for motor in motor_elements
        }
        ...
        actuated_joint_elements = [
            joint
            for joint in joint_elements
            if joint.attrib.get("name") in self._actuated_joint_names
        ]
        if len(actuated_joint_elements) != self.num_dof:
            raise ValueError(
                f"Expected {self.num_dof} actuated joints in {self.mjcf_file}, "
                f"found {len(actuated_joint_elements)}"
            )
        self.actuated_joint_names = [
            joint.attrib["name"] for joint in actuated_joint_elements
        ]
        for joint in actuated_joint_elements:
            self.dof_axis.append([float(value) for value in joint.attrib["axis"].split()])
        self.has_freejoint = tree.getroot().find("worldbody").find("body/freejoint") is not None or any(
            joint.attrib.get("type") == "free" for joint in joint_elements
        )

        self.dof_axis = torch.tensor(self.dof_axis, dtype=torch.float32)
        self.dof_axis /= torch.linalg.vector_norm(self.dof_axis, dim=-1, keepdim=True).clamp_min(
            1.0e-8
        )
        ...
        if return_full:
            ...
            if len(self.cfg.extend_config) > 0:
                ...
                if not len(self.actuated_joints_idx) == len(self.body_names):
                    actuated_pose = pose[..., self.actuated_joints_idx, :]
                    axes = self.dof_axis.to(device=pose.device, dtype=pose.dtype)
                    return_dict.dof_pos = (actuated_pose * axes).sum(dim=-1)
                else:
