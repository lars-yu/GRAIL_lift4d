...
class HOIVisualizer:
    ...
    def visualize(self, data, pred, hoi_data, vis_name, vis_cfg):
        ...
        # Both meshes must come from the same ``hoi_data`` snapshot. Using
        # ``pred`` for the object while rebuilding the human from ``hoi_data``
        # can silently mix init and final optimization stages during rerenders.
        ...
        obj_faces = motion_seq["obj_seq"]["triangles"]
        ...
        renderer = create_renderer(
            camera_for_frame(self.cameras, 0),
            image_size,
            device=self.device,
        )
        ...
        if render_video:
            ...
            extra_views = self._setup_extra_views(
                ...
                obj_verts_seq,
                data,
                scene_center=vis_cfg.get("extra_view_scene_center"),
                cam_distance=vis_cfg.get("extra_view_camera_distance"),
            )
    ...
    def _render_video(
        ...
        with imageio.get_writer(output_path, fps=self.video_fps) as writer:
            for i in tqdm(range(frame_num), desc=f"Visualizing {desc}"):
                ...
                camera_i = camera_for_frame(camera, i)
                renderer_i = (
                    create_renderer(camera_i, image_size, device=self.device)
                    if is_batched_camera(camera)
                    else renderer
                )
    ...
    def _setup_extra_views(
        ...
        *,
        ...
        if scene_center is None:
            scene_center = (
                all_verts_first.min(dim=0)[0] + all_verts_first.max(dim=0)[0]
            ) / 2
        else:
            scene_center = torch.as_tensor(
                scene_center, dtype=torch.float32, device=self.device
            ).reshape(3)
