...
class HOIData:
    ...
    palm_surface_target_world: Optional[torch.Tensor] = None  # (L,3), object-shell ray target
    ...
    grasp_surface_anchor_obj_local: Optional[torch.Tensor] = None  # (3,), shell anchor
    grasp_normal_obj_local: Optional[torch.Tensor] = None  # (3,), optional object-frame normal
    grasp_tangent_obj_local: Optional[torch.Tensor] = None  # (3,), signed palm tangent
    grasp_target_world: Optional[torch.Tensor] = None  # (L,3), object-relative post-contact target
    grasp_surface_target_world: Optional[torch.Tensor] = None  # (L,3), transported shell anchor
    grasp_target_normal_world: Optional[torch.Tensor] = None  # (L,3), rotates with object
    grasp_target_tangent_world: Optional[torch.Tensor] = None  # (L,3), rotates with object
    grasp_contact_direction_world: Optional[torch.Tensor] = None  # (3,), shell-to-hand
    grasp_center_clearance: Optional[torch.Tensor] = None  # scalar, palm centre-to-shell
    support_foot_reference_world: Optional[torch.Tensor] = None  # (L,2,3), episode anchors
    support_foot_mask: Optional[torch.Tensor] = None  # (L,2), active planted feet
    hmr_support_foot_reference_world: Optional[torch.Tensor] = None  # original HMR anchors
    hmr_support_foot_mask: Optional[torch.Tensor] = None  # original HMR support state
    footstep_plan: Any = None  # optional whole_body_ik.FootstepPlan
    adaptive_ik_mode: Optional[str] = None  # m0_upper_body/m1_stance/m2_step
    adaptive_ik_attempts: Optional[list[dict[str, Any]]] = None
