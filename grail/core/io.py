...
def load_mesh(mesh_path, mesh_scale=None, target_num_verts=None, device="cuda"):
    ...
    def remesh_to_target(trimesh_mesh, target_verts, tolerance=0.2):
        ...
        """
        original_verts = len(trimesh_mesh.vertices)

        try:
            trimesh_mesh.merge_vertices()
        except Exception as e:
            print(f"Warning: Vertex merge failed before remeshing ({e})")

        merged_verts = len(trimesh_mesh.vertices)
        if merged_verts != original_verts:
            print(f"Merged duplicate vertices before remeshing: {original_verts} -> {merged_verts}")
