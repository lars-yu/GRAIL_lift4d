    ...
    if torch.any(nind):
        ...
        if torch.any(nxind):
            out[nxind] = exp_map_to_quat(
                normalize_vec(torch.cross(vx.expand_as(v1[nxind]), v1[nxind], dim=-1)) * np.pi
            )
        ...
        if torch.any(pind):
            ...
            out[pind] = exp_map_to_quat(
                normalize_vec(torch.cross(vy.expand_as(v1[pind]), v1[pind], dim=-1)) * np.pi
            )
...
def quat_to_angle_axis(q):
    ...
    half_angles = torch.atan2(norms, q[..., qw])  # half_angles: [0,pi] because norms >= 0
